 ...
 */
@DoPrivileged
public class MyCustomLanguageImpl implements Language {
	...
	private static MyCustomLanguageImpl _getInstance() {
		Long companyId = CompanyThreadLocal.getCompanyId();

		MyCustomLanguageImpl instance = _instances.get(companyId);

		if (instance == null) {
			instance = new MyCustomLanguageImpl(companyId);
	...
	private MyCustomLanguageImpl() {
		this(CompanyConstants.SYSTEM);
	}

	private MyCustomLanguageImpl(long companyId) {
	...
	private static Log _log = LogFactoryUtil.getLog(MyCustomLanguageImpl.class);

	private static Map<Long, MyCustomLanguageImpl> _instances =
		new ConcurrentHashMap<Long, MyCustomLanguageImpl>();
